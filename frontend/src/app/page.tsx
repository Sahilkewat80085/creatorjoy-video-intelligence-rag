...
export default function Home() {
  ...
  function handleIngestSuccess(data: IngestResponse) {
    setIngestData(data);
    // Scroll smoothly to the video cards
    setTimeout(() => {
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 py-8 flex flex-col gap-8">
        ...
        <header className="text-center py-6">
          ...
          <p className="text-white/50 text-base md:text-lg max-w-xl mx-auto">
            Compare YouTube and Instagram content using retrieval-augmented generation
          </p>
        </header>

        {/* ── INGEST SECTION ── */}
        <section className="rounded-2xl border border-white/10 bg-white/3 backdrop-blur-sm p-6">
          <div className="flex items-center justify-between mb-5">
            <div>
              <h2 className="text-lg font-semibold text-white">Video Sources</h2>
              <p className="text-sm text-white/40 mt-0.5">
                Provide a YouTube and Instagram URL to begin
              </p>
            </div>
            {ingestData && (
              <button
        ...
        {/* ── CHAT ── */}
        <section ref={chatRef} className="rounded-2xl border border-white/10 bg-white/3 backdrop-blur-sm overflow-hidden" style={{ minHeight: "520px", display: "flex", flexDirection: "column" }}>
          <div className="px-6 py-4 border-b border-white/10 flex items-center justify-between shrink-0">
            ...
            <div className="flex items-center gap-2 text-xs text-white/30 border border-white/10 rounded-full px-3 py-1">
              <div className={`w-1.5 h-1.5 rounded-full ${ingestData ? "bg-emerald-400 animate-pulse" : "bg-white/20"}`} />
